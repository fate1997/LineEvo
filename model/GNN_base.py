...
class GATv2(nn.Module):
    ...
    def forward(self, data: Data):
        ...
        if self.readout.name == 'LineEvo':
            mol_repr = self.readout_func(data) # x, edge_index, edge_attr, pos, batch
            mol_repr_all += mol_repr
        if self.readout.name == 'LineFlat':
            mol_repr = self.readout_func(x, edge_index, data.triple_index, data.quadra_index, pos, batch, mol_repr_all)
            mol_repr_all = mol_repr
    ...
    def get_readout(self, readout_config):
        ...
        if readout_name == 'LineEvo':
            ...
            pass
            # raise ValueError("The implented readout functions are [LineEvo, LineFlat, Add]")
